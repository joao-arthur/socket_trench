...
const APP_ID: &str = "com.joao_arthur.socket_trench";
...
fn copy_icons_and_create_path_if_needed(_source: &str, _destination: &str) {
...
fn update_icon_cache(location: &str) {
    let status = Command::new("gtk-update-icon-cache")
        .args(&[location, "--ignore-theme-index"])
        .status()
